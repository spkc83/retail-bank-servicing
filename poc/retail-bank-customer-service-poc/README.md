...
for OOD, classifier failure, and policy no-match paths without relevant tool
evidence. A policy no-match turn whose question is answered by recent tool
results runs one grounded converse pass instead (tagged
`fallback:policy-no-match-grounded-converse` in constraint notes).
