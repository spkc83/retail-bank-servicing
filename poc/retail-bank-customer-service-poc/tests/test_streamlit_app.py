...
from streamlit_app import resolve_local_router_artifact
...
def test_local_streamlit_prefers_the_verified_release_router_artifact() -> None:
    ...
    assert artifact.name == "banking-conversation-router-v4-release"
...
def test_streamlit_app_renders_local_login_without_loading_model(
    ...
    assert any("Local Granite 9B" in title.value for title in app.title)
...
def test_experiment_diagnostics_use_an_on_demand_sidebar_popover() -> None:
    ...
    assert (
        'st.expander("Model, router, tool-call, and raw-output diagnostics"'
        not in source
    )
