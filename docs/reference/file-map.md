...
| [`../../src/hello_slm/banking_corpus_coverage.py`](../../src/hello_slm/banking_corpus_coverage.py) | Classifies corpus rows by use-case category and phrasing form; evaluates the declared coverage matrix. |
| [`../../scripts/retail_bank/measure_corpus_coverage.py`](../../scripts/retail_bank/measure_corpus_coverage.py) | Reports coverage against `configs/corpus-coverage.toml`; `--gate` fails below a declared minimum. See [Use-case coverage](../18-corpus-usecase-coverage.md). |
