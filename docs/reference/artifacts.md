...
| PEFT run blocked by the gate | v13 (job `6a9261e4`, seed 9151, dataset `@8494c94f`) trained to completion and was refused publication by the bare-probe behavioural gate (poem, change_pin, balance); the destination repository was never created. Behaviour stability across v11/v12/v13: request-status and statement boundary 3/3, PIN boundary 2/3, poem refusal 1/3, balance honesty 0/3 — the alignment families need weighting in the training mix, not another run |
