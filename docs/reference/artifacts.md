...
| PEFT candidate (gated, published, NOT deployed) | `spkc83/retail-bank-servicing-agent-9b-peft-v14-prompt-realized@47968b2b9ce02973b5676e464aafaa768cdbb05e` — v14 on dataset `@ce0d4429`, source `4c7e986`, seed 7303, 2000 steps, `--skip-merge-adapter`. **The first run to clear all three gates**: coreference dev and shadow at 1.0 on every metric, and bare probes 11 of 11 with no failures, including the poem refusal and the no-evidence balance claim that v12 lost. `eval_loss` 0.5594 (v12: 0.5605). Deployment is a separate decision and has not been made; v11 remains the deployed adapter. Publication was completed by hand from the job bucket after the in-job upload stalled — see [the run note](../superpowers/plans/2026-08-29-improvement-register.md) |
