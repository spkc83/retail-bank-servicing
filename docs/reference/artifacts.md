...
| ZeroGPU Space source/pins | `spkc83/retail-bank-servicing-poc@453a227f2395747ec0635bb6c298c050f006aedd` |
...
| Component | Identity |
| --- | --- |
| PEFT release (previously deployed) | `spkc83/retail-bank-servicing-agent-9b-peft-v11-alignment@03a7b44633fadab7ad672b009925cc68b52494d4` — v11 policy-alignment retrain (job `6a908078`, dev and shadow gates 1.0, dataset `@b5ec0489`, source `6e53ebf`); adapter_config.json lives under `adapter/`, so the runtime needs `RETAIL_BANK_ADAPTER_SUBFOLDER=adapter` |
| PEFT release (deployed) | `spkc83/retail-bank-servicing-agent-9b-peft-v14-prompt-realized@47968b2b9ce02973b5676e464aafaa768cdbb05e` — v14 on dataset `@ce0d4429`, source `4c7e986`, seed 7303, 2000 steps, `--skip-merge-adapter`. **The first run to clear all three gates**: coreference dev and shadow at 1.0 on every metric, and bare probes 11 of 11 with no failures, including the poem refusal and the no-evidence balance claim that v12 lost. `eval_loss` 0.5594 (v12: 0.5605). Deployed to the Space on 2026-09-01 (Space commit `453a227f`), replacing v11. Publication was completed by hand from the job bucket after the in-job upload stalled — see [the run note](../superpowers/plans/2026-08-29-improvement-register.md) |
