...
| --- | --- | --- | --- |
| 1 | **CLOSED** — **From-scratch training is not reproducible, and the seed knob records determinism it does not provide.** `seed_training()` is never called by `run_remote_training()`; `SFTConfig` sets neither `seed=` nor `data_seed=`. `TRAINING_SEED` reaches only the mix path, which short-circuits when all multipliers are 1 (the default). The value still lands in the fingerprint. **This is the most plausible explanation for the v11→v12 behaviour churn** — not dosing. *Closed: `run_remote_training` calls `seed_training(TRAINING_SEED)` before `build_training_configs`, which sets both `seed` and `data_seed`; a test pins the ordering. The "do not spend on another training run" hold below no longer applies.* | `cloud_train_tool_sft.py:1208`, `:909-910`, `tests/test_banking_tool_sft_worker.py:859` | S |
| 2 | **CLOSED** — **Router-unavailable degrades to maximum tool authority** — all 9 tools including 4 mutations, no turn guidance, no entity grounding. Verified: `router=None` exposes `freeze_card/replace_card/dispute_transaction/cancel_transfer`. A genuine `uncertain` route is safe; only the degraded path fails open. *Closed: `_generation_plan` treats an absent `action` as most-restrictive and renders clarify guidance with no tools. The unrouted surface survives as a measurement baseline but a caller must now ask for it by name (`tool_authority == "unrouted"`).* | `model_service.py:1299-1320` | S |
| 3 | **CLOSED** — **One prior tool message disables the zero-tool claim guard for the whole session.** Verified: with a `role=="tool"` message in history, `"I've frozen your card ending 6101…"` passes; without it, correctly rejected. Fires on exactly the dangerous turn. A test pins the permissive behaviour. *Closed: a completed-action claim is now held to the tool that performs it, so a read grounds no mutation claim in either the current turn or the history. `evidence_tools` carries the names past raw execution envelopes, which have none. An offered action ("if you'd rather I froze that card") is exempt by clause, which the shipped corpus needs.* | `response_policy.py:325-395`, `:551-620`, `model_service.py:576,634` | M |
| 4 | **PARTIAL, and the third clause was imprecise** — **No PII gate in the largest generator**; **router PII is report-only**; **cross-split leakage gate structurally cannot fire** (32/35 test rows share a 4-gram with train). *The first two halves closed at `428bcb6`. On the third: `split_group` embeds the state-seed index and the splitter assigns splits by that same index, so on a correct corpus the gate cannot fire — but that is what it is for. It certifies the splitter, not the content, and it does fire when a group straddles a split (`test_the_split_group_gate_fires_when_a_group_straddles_two_splits`). The 4-gram overlap is a different problem that this gate never claimed to cover; it belongs to the contamination work below, and exact user turns no longer overlap train and test.* | `banking_servicing_alignment_data.py:4820-4829`, `tests/test_detector_injection.py` | M |
| 5 | **CLOSED** — **No test asserts any detector fires** (13 assertions, all `== 0` on clean fixtures). *`tests/test_detector_injection.py` plants a violation for each of the three PII gates, for the split-group leakage gate and for the duplicate-current gate, and asserts rejection in each case. `tests/test_committed_digests.py` covers the provenance side.* | `tests/test_detector_injection.py` | M |
| 6 | **CLOSED** — **No test recomputes any committed dataset or lock digest**, and the upstream lock check is off by default with a missing lock file. One v4 lock records three digests matching nothing in the repo. *Closed: `tests/test_committed_digests.py` rehashes every split named by every manifest and every lock, checks the recorded row counts against the files, and requires each corpus to carry a manifest. It found the v4 lock on its first run and confirmed the register's claim by recomputation. The corpus is the honest artifact and the lock is the stale one — `banking-counterfactual-eval-v1`'s `source_inputs` independently pins the v4 train split at the digest the file actually has — so the lock is left alone and the discrepancy is asserted by `test_the_stale_v4_lock_stays_visible`, which fails if either side quietly moves.* | `tests/test_committed_digests.py` | S |
| 7 | **OPEN, re-verified** — **Two "semantics unchanged" hashes validate a value against itself** — the tool-SFT hash excludes exactly the two messages it is recomputed after overwriting; the ASR digest compares a deepcopy against its source with the mutated field masked. *Both confirmed unchanged: `_immutable_record_hash` still excludes `last_user_index` and `final_assistant_index`, the two messages the realization passes overwrite, and `_semantic_digest` still masks the last user message to a placeholder before hashing both source and copy. Whether the exclusion is a defect or the design still needs an argument: the exclusion is what lets a teacher rewrite wording at all, so the question is whether anything else proves those two fields moved only as intended.* | `banking_tool_sft_data.py:3195-3223`, `banking_asr_sft_data.py:318-330` | S / M |
| 8 | **CLOSED, on the second attempt** — **Three fail-open paths in dataforge**: split-name typo disarms guards; `record_id_field` means two different things across modules and reports a clean batch; `manifest_extra` can overwrite the governance report. *First marked closed on 2026-09-11 on the strength of a commit message named "close the fail-open paths"; a verification pass against the source found all three intact. That is the same mistake this register exists to catch, recorded rather than tidied away. Actually closed at `5fe368d`: guards validate explicitly-passed split names against the corpus, `manifest_extra` raises on a reserved key, and the checker reads the literal wire key for wire rows while `record_id_field` applies to records only. Each has a test that plants the mistake.* | sft-dataforge `src/dataforge/{guards,emit,checks}.py` | S each |

## P1 — reproducibility, provenance, and cost safety

| # | Finding | Where | Effort |
| --- | --- | --- | --- |
| 9 | **PARTIAL** — **The from-scratch fingerprint omits every hyperparameter that changes the model** — lr, steps, batch, grad-accum, seq-len, warmup, multipliers, source commit. Two materially different runs fingerprint identically, and resume will load a mismatched checkpoint. The continuation fingerprint already carries all of it. *Closed at `07b7df2`: `training_fingerprint` now carries `optimization` (max steps, max train seconds, learning rate, batch size, accumulation, sequence length, warmup ratio) and `training_mix` (all four multipliers). Source commit is the one item of the original list still left out, and the two lanes diverge on it: the continuation lane requires it through `require_exact_revision` and puts it in its fingerprint, while the from-scratch lane reads it with a silent `"unrecorded"` fallback and uses it only in the model-card markdown. Until that closes, two from-scratch runs from different commits still fingerprint identically.* | `cloud_train_tool_sft.py:611-640`, `:1091`, `cloud_continue_tool_sft.py:894-900` | S |
| 10 | **CLOSED** — **Publication is five non-atomic commits**; a partial failure leaves weights with no metadata, indistinguishable from a complete release, and the retry then dies on the non-empty-destination check. Both other lanes already use a single `create_commit`. *Closed at `b68515f`: the module holds exactly one `create_commit` call. The follow-on dotfile defect that commit's own test caught is fixed at `4c7e986`.* | `cloud_train_tool_sft.py:1453` | M |
| 11 | **CLOSED** — **The billed-job guard cannot gate spend**: the launcher submits after format validation only, the real guard runs inside the already-billed container, and `JOB_TIMEOUT` defaults to 5h on a $2.75/h flavour with no ceiling (`999h` validates). *Closed at `4c526fe`: the launcher prices the worst case from timeout times flavour rate before submitting, refuses above `MAX_JOB_COST_USD` (default $5) and refuses without `CONFIRM_SPEND=1`. There is still no ceiling on the hours themselves, because the cost ceiling is the binding one. `DRY_RUN=1` validates and prices without submitting.* | `run_remote_training_job.sh:60-95` | S |
...
| 13 | **OPEN, re-verified** — `run_bare_probe_gate` is imported and called only in `cloud_train_tool_sft.py`, with no call anywhere in the continuation module, and `continuation_module()` still lazy-imports to dodge the circular import. — **Gate wiring is asymmetric**: the bare-probe gate is train-lane only, so the continuation lane can publish without it; the coreference gates live in the continuation module and are reached through a circular-import shim. | `cloud_train_tool_sft.py:443-456,1333`, `cloud_continue_tool_sft.py:1409-1416` | M |
...
| 21 | **PARTIAL** — Dataforge packaging is not publishable: no LICENSE, no `py.typed` (a mypy-clean library shipping invisible types), deprecated license form, no version/CHANGELOG/CI/CLI. *LICENSE, `py.typed` and the PEP 639 license fields are in, verified by building a wheel and reading both files back out of it. A version is set. No CHANGELOG and no CLI; CI stays declined.* | sft-dataforge `pyproject.toml`, `LICENSE`, `src/dataforge/py.typed` | S |
...
- **CLOSED (behavioural check, rescored v14 11/11).** The OOD / small-talk response-path gate passes only answers containing the literal `retail banking` (`banking_tool_eval.py:701-715`). All 142 out-of-domain train finals contain the phrase, but the deployed v14 adapter scores 0/11: it declines all eleven and redirects to banking without reproducing the phrase. The gate checks a trained string, not the behaviour, and it is the only check that an out-of-domain answer declines (`ood_false_accept` counts tool calls only). Replacing it with a behavioural check changes a release gate that published v8 scores were measured under, so it is a decision rather than a patch. Replaced by a behavioural check (no tool call, no action claim, secret or advice, an opening deflection about the assistant's reach, a redirect naming the banking scope) calibrated on all 200 trained out-of-domain finals; a first, looser version passed 16 of 16 planted bad answers and was tightened before release; a second set of 34 bad answers written without sight of the tests leaves 1 passing (non-numeric medical advice), documented in docs/06. Rescoring v14's saved predictions gives 11/11. Still open: `_credential_request` misses "share your full card number and PIN" and "your online banking password", because `full` and `online banking` break its pattern; and all eleven out-of-domain test prompts are about the weather. `banking_tool_eval.py` `_declines_out_of_domain` · S
- **CLOSED (every target scored; the job fails at the end).** `hf_job_tool_eval.py` runs its evaluation targets sequentially with `check=True`, so the first target whose enforced gate fails stops the job and later targets are never scored. Both surfaced in the v14 score on the rotated fixture (job 6aae07ea). · S
...
> **Status of that order, re-verified against the code on 2026-09-11 and 2026-09-12.**
> Closed: 1, 2, 3, 5, 6, 8, 10, 11. Partly closed: 4, 9, 12, 14, each row saying what remains.
> Still open: 7 and 13.
>
> The training-spend hold is lifted, both fail-open runtime paths are shut, every detector named
> in the audit has a test that plants its violation, every committed digest is recomputed on each
> run, and the three dataforge fail-open paths are actually closed. What is left is finding 7,
> which needs an argument rather than a patch; finding 13's missing bare-probe gate on the
> continuation lane; and the source commit that finding 9 still omits from the from-scratch
> fingerprint.
>
> **Read this as a warning, not a summary.** Two of these rows were wrong in both directions
> within a day. Findings 1, 2, 9, 10 and 11 sat marked open for two weeks after they were fixed,
> while the prose below recorded the fixes the tables did not. Then finding 8 was marked closed
> here on the strength of a commit message, and a pass against the source found all three of its
> paths intact. A row is worth what its inline evidence is worth. Re-check the code.
