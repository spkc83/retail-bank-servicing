...
| 7 | **NOT RE-VERIFIED** — **Two "semantics unchanged" hashes validate a value against itself** — the tool-SFT hash excludes exactly the two messages it is recomputed after overwriting; the ASR digest compares a deepcopy against its source with the mutated field masked. *Whether the exclusion is the defect or the design needs an argument, not a grep, so this row was left alone rather than guessed at.* | `banking_tool_sft_data.py:589-590`, `banking_asr_sft_data.py:229` | S / M |
| 8 | **CLOSED** — **Three fail-open paths in dataforge**: split-name typo disarms guards; `record_id_field` means two different things across modules and reports a clean batch; `manifest_extra` can overwrite the governance report. *Closed in the dataforge repository at `611122b`, "close the fail-open paths in the v11 guards".* | sft-dataforge `src/dataforge/` | S each |

## P1 — reproducibility, provenance, and cost safety

| # | Finding | Where | Effort |
| --- | --- | --- | --- |
| 9 | **CLOSED** — **The from-scratch fingerprint omits every hyperparameter that changes the model** — lr, steps, batch, grad-accum, seq-len, warmup, multipliers, source commit. Two materially different runs fingerprint identically, and resume will load a mismatched checkpoint. The continuation fingerprint already carries all of it. *Closed at `07b7df2`: `training_fingerprint` now carries `optimization` (max steps, max train seconds, learning rate, batch size, accumulation, sequence length, warmup ratio) and `training_mix` (all four multipliers). Source commit is still not in it, which is the one item of the original list left out.* | `cloud_train_tool_sft.py:611-640` | S |
...
> **Status of that order, re-verified against the code.** Closed: 1, 2, 3, 5, 6, 8, 9, 10, 11.
> Partly closed: 4, 12 and 14, each row saying what remains. Still open: 13. Not re-verified: 7,
> because whether the hash exclusion is a defect or the design is an argument rather than a grep.
>
> The P0 block is effectively done. The training-spend hold is lifted, both fail-open runtime paths
> are shut, every detector named in the audit now has a test that plants its violation, and every
> committed digest is recomputed on each run. What is left under P0 is finding 7, which needs an
> argument, and the wording of finding 4's third clause, corrected in place above.
>
> Every CLOSED row carries its evidence inline. Re-check it against the code rather than trusting
> this line — these tables had drifted from the code for two weeks before this pass, while the
> prose below recorded fixes the tables did not.
