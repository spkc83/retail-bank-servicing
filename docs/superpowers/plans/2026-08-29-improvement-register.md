...
| 4 | **PARTIAL** — **No PII gate in the largest generator**; **router PII is report-only**; **cross-split leakage gate structurally cannot fire** (32/35 test rows share a 4-gram with train). *The first two halves are closed at `428bcb6`. The cross-split gate is still keyed on a value that embeds the split, so it still cannot fire.* | `banking_servicing_alignment_data.py:4738-4747` | M |
| 5 | **PARTIAL** — **No test asserts any detector fires** (13 assertions, all `== 0` on clean fixtures). *`tests/test_detector_injection.py` plants a violation for each of the three PII gates and asserts rejection. The leakage and duplicate detectors still have no fire test.* | `tests/test_detector_injection.py`, `tests/test_banking_*_data.py` | M |
...
> **Status of that order, re-verified against the code.** Closed: 1, 2, 3, 6, 8, 9, 10, 11. Partly
> closed: 4 and 5 (the PII half landed, the leakage half did not), 12 and 14 as their rows already
> say. Still open: 13. Not re-verified: 7, because whether the hash exclusion is a defect or the
> design is an argument rather than a grep.
>
> The training-spend hold is therefore lifted and both fail-open runtime paths are shut. What is
> left of the original P0 block is two pieces of verification work: the cross-split leakage gate
> that still cannot fire, and fire tests for the leakage and duplicate detectors. Every CLOSED row
> carries its evidence inline. Re-check it against the code rather than trusting this line — these
> tables had drifted from the code for two weeks before this pass, while the prose below recorded
> the fixes the tables did not.
