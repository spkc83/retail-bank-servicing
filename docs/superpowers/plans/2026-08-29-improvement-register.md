...
| 6 | **OPEN** — **No test recomputes any committed dataset or lock digest**, and the upstream lock check is off by default with a missing lock file. One v4 lock records three digests matching nothing in the repo. *Re-checked: tests read committed corpus content but none recomputes a manifest or lock digest and compares it. `check_corpora_reproduce.py` does exactly this, but it is a script reached through `make corpora`, so it binds only when someone runs it — see #14.* | `prepare_servicing_alignment_data.py:18`, `data/sources/*.lock.json` | S |
...
> **Status of that order, re-verified against the code.** Closed: 1, 2, 3, 8, 9, 10, 11. Partly
> closed: 4 and 5 (the PII half landed, the leakage half did not), 12 and 14 as their rows already
> say. Still open: 6 and 13. Not re-verified: 7, because whether the hash exclusion is a defect or
> the design is an argument rather than a grep.
>
> The training-spend hold is therefore lifted and both fail-open runtime paths are shut. What is
> left of the original P0 block is verification work: the cross-split leakage gate that still
> cannot fire, fire tests for the leakage and duplicate detectors, and a test that recomputes a
> committed digest. Every CLOSED row carries its evidence inline. Re-check it against the code
> rather than trusting this line — these tables had drifted from the code for two weeks before
> this pass, while the prose below recorded the fixes the tables did not.
