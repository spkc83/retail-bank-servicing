...
- The OOD / small-talk response-path gate passes only answers containing the literal `retail banking` (`banking_tool_eval.py:701-715`). The deployed v14 adapter scores 0/11 on it while declining all eleven correctly, because its teacher-realized voice never says that phrase; the gate measures the old fixed refusal string. Relatedly, `hf_job_tool_eval.py` runs its evaluation targets sequentially with `check=True`, so the first target whose enforced gate fails stops the job and later targets are never scored. Both surfaced in the v14 score on the rotated fixture (job 6aae07ea). · S
